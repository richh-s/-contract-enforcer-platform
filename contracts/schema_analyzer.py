...
# Contracts may use either "models" (newer generator) or "schema" (older generator)
REQUIRED_CONTRACT_KEYS = {"kind", "apiVersion"}
...
def validate_contract_structure(contract: dict, path: Path) -> None:
    ...
    Accepts both generator output formats:
      - newer generator: uses 'models' key
      - older generator: uses 'schema' key (still valid Bitol ODCS)
    """
    ...
    has_models = isinstance(contract.get("models"), dict)
    has_schema = isinstance(contract.get("schema"), dict)
    if not has_models and not has_schema:
        raise ValueError(
            f"Snapshot '{path}': must have either 'models' (newer format) or "
            "'schema' (older format) as a dict. Got neither."
        )
...
def extract_model_fields(contract: dict) -> tuple[str, dict]:
    ...
    Handles both generator output formats:
      - newer: contract["models"][name]["fields"]
      - older: contract["schema"]  (flat field dict at top level)
    """
    # Newer format
    ...
    # Older format — schema: is a flat field dict
    schema = contract.get("schema", {})
    if schema and isinstance(schema, dict):
        return "schema", flatten_fields(schema)
...
def compute_blast_radius(
    ...
    """
    Compute blast radius using the ContractRegistry as the PRIMARY source.
    ...
    Step 1 — Registry (primary): query ALL registered contracts for
      consumers who declare any breaking field in fields_consumed or
      breaking_if_changed.
    Step 2 — Lineage (enrichment): merge consumers declared in the
      contract's lineage.downstream that weren't found via the registry.
    """
    from contracts.registry import ContractRegistry  # lazy import avoids circular deps
    ...
    try:
        ...
        blast    = registry.query_blast_radius(
            contract_id=contract_id,
            breaking_fields=breaking_fields,
            lineage_downstream=downstream,
        )
        blast["source_lineage_snapshot"] = lineage.get("snapshot_id")
        blast["total_downstream_nodes"]  = len(downstream)
        return blast
    except Exception as exc:
        # Registry unavailable — fall back to lineage-only (graceful degradation)
        affected = []
        for node in downstream:
            ...
            if breaking_fields & (consumed | breaking):
                affected.append(node.get("id", "unknown"))
        return {
            "affected_nodes":           affected,
            "affected_pipelines":       affected,
            "affected_consumers_count": len(affected),
            "source_lineage_snapshot":  lineage.get("snapshot_id"),
            "total_downstream_nodes":   len(downstream),
            "blast_radius_method":      f"lineage_fallback (registry error: {exc})",
        }
...
def build_report(
    ...
    blast   = compute_blast_radius(new_contract, breaking_fields, contract_id=contract_id)
